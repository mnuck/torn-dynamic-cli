...
### Check for payout-chain predecessors

If a crime appears in the late-oc window, check if it has a `previous_crime_id`. If it does, investigate the predecessor to see if it was also delayed.

```bash
# Command to check a specific crime
./torn faction crime --crimeId <id> | python3 -c "
import json, sys
try:
    c = json.load(sys.stdin)['crime']
    ra, ea = c.get('ready_at'), c.get('executed_at')
    delay = (ea - ra) if ea else None
    print(f'Name: {c.get(\'name\')} | Difficulty: {c.get(\\'difficulty\\')}')
    print(f'Previous Crime ID: {c.get(\\'previous_crime_id\\')}')
...
After checking predecessors, **always also run Phase 3** to check for upcoming OCs at risk. Do not wait for the user to ask.
...
Use the built-in CLI report command instead of manually looping API calls:

```bash
./torn report oc-risk
```

This command fetches all planning OCs, fetches the entire faction member list in a **single** call (not one call per member), filters to OCs whose `ready_at` falls within the next 6 hours, and flags any member whose status is not "Okay" (Abroad, Traveling, Hospital, Jail). It's the fast, correct replacement for the old per-member `torn user profile` loop — use it instead of hand-rolling the fetch/filter logic below.
...
Name                      Ready at   Risks                    
------------------------------------------------------------
Market Forces             10:03 UTC
  └─ ⚠ coolcookie123 (Traveling from United Kingdom to Torn)
  └─ ⚠ BLACK-N0VA (Traveling from UAE to Torn)
```

If it reports "No upcoming OCs are currently at risk", trust it — don't re-verify with individual `user profile` calls.
...
1. Run `./torn report oc-risk` to get `ready_at`, member list, and flagged at-risk members for every OC in the next 6 hours
2. For each flagged member (Abroad or Traveling), estimate whether they can make it back in time using the travel-time reference below

### Travel time reference

If a member is abroad, estimate their return time based on their flight class. Use the **Business Class** time for the fastest possible return and the **Standard** time for the slowest.

If the member has an active effect from the book **"Mailing Yourself Abroad"** (the 31-day travel time reduction), apply a **25% reduction** to the travel times below.

| Destination | Standard | Airstrip | WLT | Business |
| :--- | :--- | :--- | :--- | :--- |
| Mexico | 26m | 18m | 13m | 8m |
| Cayman Islands | 35m | 25m | 18m | 11m |
| Canada | 41m | 29m | 20m | 12m |
| Hawaii | 2h 14m | 1h 34m | 1h 07m | 40m |
| United Kingdom | 2h 39m | 1h 51m | 1h 20m | 48m |
| Argentina | 2h 47m | 1h 57m | 1h 23m | 50m |
| Switzerland | 2h 55m | 2h 03m | 1h 28m | 53m |
| Japan | 3h 45m | 2h 38m | 1h 53m | 1h 8m |
| China | 4h 02m | 2h 49m | 2h 1m | 1h 12m |
| United Arab Emirates | 4h 31m | 3h 10m | 2h 15m | 1h 21m |
| South Africa | 4h 57m | 3h 28m | 2h 29m | 1h 29m |
